...
export const guildSettings = {
    ...
    setQuarantineRole(db, guildId, roleId) {
        db.prepare(`INSERT INTO guild_settings (guild_id, quarantine_role_id)
       VALUES (?, ?)
       ON CONFLICT(guild_id) DO UPDATE SET quarantine_role_id=excluded.quarantine_role_id`).run(guildId, roleId);
    },
    setAntiRaid(db, guildId, input) {
        db.prepare(`INSERT INTO guild_settings (guild_id, anti_raid_enabled, anti_raid_joins_per_min)
       VALUES (?, ?, ?)
       ON CONFLICT(guild_id) DO UPDATE SET anti_raid_enabled=excluded.anti_raid_enabled, anti_raid_joins_per_min=excluded.anti_raid_joins_per_min`).run(guildId, input.enabled ? 1 : 0, input.joinsPerMin);
    }
};
